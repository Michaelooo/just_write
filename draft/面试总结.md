...
代码层面： 使用打包工具，treeshaking、代码 uglify、
...
无状态的组件： 就是组件内部不会有state变化的变化的组件

纯函数： 纯函数是确定的state props 只会渲染出确定的component， 常用于 react 的性能优化。原理是，
...
Http2 在浏览器请求过程中的好处就是，在同一个域下的请求只需要建立一个，可以提升性能，没有 http1.1 并发6 条的限制，这利用了 http2 多路复用的特性。另外还有 修改了数据传输分帧、服务推送的特性。
